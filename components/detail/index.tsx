...
import { DispatchType, StateType } from "@/redux/store";4
import { setStorage, useStorage } from "@/utils/storage";
...
const DetailNews: FC<IProps> = ({ user }) => {
    ...
    const storage = useStorage("readlist");

    const handleClick = useCallback((e:MouseEvent,author:string) => {
        e.stopPropagation();
        if(storage != null && storage.length > 0){
            const total = Array.from(new Set([...storage,...list]));
            dispatch(toggle(total)) 
        }else {
            dispatch(toggle([author]));
            setStorage('readlist',[author]);
        }
    },[storage,dispatch,list])
        ...
        <div className="detail-news">
            ...
            <figure>
                ...
                <figcaption>
                    ...
                    <div className="info">
                        <div className="read">
                            <span onClick={(e) => handleClick(e, user.author)}>{list.includes(user.author) ?
                                <>
